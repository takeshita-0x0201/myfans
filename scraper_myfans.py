...
def _scrape_user_page(page, username: str) -> dict:
    """pageオブジェクトで1ユーザー分を処理"""
...
def scrape_all_myfans(usernames: list[str]) -> dict[str, dict]:
    """全ユーザーを1ブラウザセッションでスクレイピング"""
    cookies = load_cookies('myfans')
    results = {}

    def page_action(page):
        for idx, username in enumerate(usernames):
            print(f'  [{idx+1}/{len(usernames)}] MyFans: {username}')
            try:
                data = _scrape_user_page(page, username)
                results[username] = data
                print(f'    -> {data["name"]} | {data["followers"]} followers | {data["posts"]} posts')
            except Exception as e:
                print(f'    -> ERROR: {e}')
                results[username] = _make_empty_data(username)
            time.sleep(2)
    ...
    try:
        fetcher.fetch(
            'https://myfans.jp',
            headless=True,
            timeout=120000 + len(usernames) * 60000,
        ...
        print(f'  MyFans browser error: {e}')
        # 未処理のユーザーに空データを設定
...
def scrape_user_profile(username: str) -> dict:
    """ユーザープロフィールをスクレイピング（後方互換）"""
    results = scrape_all_myfans([username])
    return results.get(username, _make_empty_data(username))
